...
struct ib_pool_device {
	struct backport_ib_device *ib_dev;
	struct ib_pd		*ib_pd;
	struct kref		ref;
...
static inline void dev_free(struct kref *ref)
{
	struct backport_ib_device *BKP_dev;
	struct ib_device_pool *pool;
...
static inline int ib_pool_dev_put(struct ib_pool_device *dev)
{
	return kref_put(&dev->ref, dev_free);
}

static inline int ib_pool_dev_get(struct ib_pool_device *dev)
{
	return kref_get_unless_zero(&dev->ref);
}

static inline bool
ib_pool_dev_exists(ORIGINAL_ib_device *ORIG_dev,
		   struct ib_device_pool *pool)
{
	struct ib_pool_device *dev;
	bool found = false;
	...
	list_for_each_entry(dev, &pool->list, entry) {
		if (dev->ib_dev->ORIG_dev == ORIG_dev) {
			found = true;
			break;
		}
	}
...
ib_pool_dev_find_get_or_create(ORIGINAL_ib_device *ORIG_dev,
			       struct ib_device_pool *pool)
{
	struct backport_ib_device *BKP_dev;
	struct ib_pool_device *dev;
	...
	mutex_lock(&pool->mutex);
	list_for_each_entry(dev, &pool->list, entry) {
		if (dev->ib_dev->ORIG_dev->node_guid == ORIG_dev->node_guid &&
		    ib_pool_dev_get(dev))
			goto out_unlock;
	}
	...
	BKP_dev = kzalloc(sizeof(*BKP_dev), GFP_KERNEL);
	if (unlikely(!BKP_dev))
		goto out_free_dev;

	err = ib_query_device(ORIG_dev, &BKP_dev->attrs);
	if (unlikely(err))
		goto out_free_BKP_dev;

	/* Embed backport device */
	BKP_dev->ORIG_dev = ORIG_dev;
	BKP_dev->name = ORIG_dev->name;
	BKP_dev->num_comp_vectors = ORIG_dev->num_comp_vectors;

	kref_init(&dev->ref);
	dev->pool = pool;
	dev->ib_dev = BKP_dev;
	dev->ib_pd = ib_alloc_pd(BKP_dev, pool->pd_flags);
